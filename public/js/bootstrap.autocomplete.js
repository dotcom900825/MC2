!function($){
  ...
  * ================================= */
  ...
  var Autocomplete = function (element, options) {
    ...
    this.afterSelect = this.options.afterSelect || this.afterSelect
    this.listen()
  }
  ...
  , setValue:function(item){
		return {"data-value":item.toString(),"real-value":item.toString(),"data-pic":item.toString()};
    }
  
  , select: function (item) {
    var val = this.$menu.find('.active').attr('data-value')
	  var realVal = this.$menu.find('.active').attr('real-value')
    var img = this.$menu.find('.active').attr('data-pic')
    img = (!img) ? " ": img
    this.$element
        .val("").attr("real-value",realVal)
        .change()
    this.afterSelect(val,realVal,img);
  ...
  ,afterSelect: function(name,id,img){
    var isUnique = 0; 
    this.$element.closest("li").siblings("li").each(function(i){
      if (id == $(this).attr("id")) isUnique = id;
    });
    if (isUnique == 0) {
      var li = '<li id="' + id + '" > \
      ...
      <div class="name"> \
      <span>' + name + '</span> \
      <a href="#" class="remove" data-remove="' + id + '">×</a> \
      </div> \
      </li>';
      $(".newmember").before(li);
      var users = $('input[name="users"]').val();
      $('input[name="users"]').val(users+','+id);
    } else{
      $("li#"+isUnique).css("background","red");
    }
  }
  ...
  , highlighter: function (item) {
  	  var that = this
  	  var name = that.formatItem(item).replace(/<\/?.+?>/g,"")
      var img = that.formatItem(item).replace(name,"")
      var query = this.query.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g, '\\$&')
      var highlight = name.replace(new RegExp('(' + query + ')', 'ig'), function ($1, match) {
        return '<strong style="color:#2ECC71;">' + match + '</strong>'
      })
