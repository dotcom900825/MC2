...
(function($){
  ...
  $.fn.tagsManager = function (options,tagToManipulate) {
    ...
    var typeaheadSelectedItem = function () {
      ...
      return typeahead_data ? typeahead_data.$menu.find(listItemSelector) : typeahead_data;
    ...
    var ajaxPolling = function (query, process) {
      if (typeof (tagManagerOptions.typeaheadAjaxSource) == "string") {
        var token = $('.form-horizontal input[name="authenticity_token"]').val();
        $.ajax({
          cache: false,
          type: "POST",
          contentType: "application/json",
          dataType: "json",
          url: tagManagerOptions.typeaheadAjaxSource,
          data: JSON.stringify({ search: query, authenticity_token: token}),
          success: function (data) { console.log(data); onTypeaheadAjaxSuccess(data, false, process); }
        });
      }
    };
